...
class NewLanding extends React.Component {
  ...
  toggleState = (e) => {
    this.setState(
      {...defaultState, [e.target.dataset.name]: true}
    )
  }
  ...
  renderUseApp = () => {
    if (this.state.login) {
      return (
        <div>
          <h1>login</h1>
          <div data-name="signup" onClick={this.toggleState} className="signup-or-in" id="signup-button">Sign Up</div>
        </div>
      )
      ...
      return (
        <div>
          <h1>signup</h1>
          <div data-name="login" onClick={this.toggleState} className="signup-or-in" id="login-button">Login</div>
        </div>
      )
    } else {
      return (
